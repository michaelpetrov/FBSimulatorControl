...
+ (instancetype)withPath:(NSString *)path scale:(CGFloat)scale logger:(id<FBSimulatorLogger>)logger eventSink:(id<FBSimulatorEventSink>)eventSink
{
  return [[self alloc] initWithPath:path scale:scale logger:logger eventSink:eventSink];
}

- (instancetype)initWithPath:(NSString *)filePath scale:(CGFloat)scale logger:(id<FBSimulatorLogger>)logger eventSink:(id<FBSimulatorEventSink>)eventSink
{
  self = [super init];
  if (!self) {
    return nil;
...
- (void)framebuffer:(FBSimulatorFramebuffer *)framebuffer didGetSize:(CGSize)size
{
  dispatch_async(self.mediaQueue, ^{
    ...
    self.size = size;
    [self startRecordingToFileWithPath:self.filePath error:nil];
  });
}
...
- (BOOL)startRecordingToFileWithPath:(NSString *)path error:(NSError **)error
{
  if (!path) {
    return [[FBSimulatorError describe:@"No path provided"] failBool:error];
  }
  ...
  return [self createAssetWriterAtPath:path size:self.size error:error];
}
...
- (void)teardownWriter
{
  ...
  [writer finishWritingWithCompletionHandler:^{
    dispatch_async(dispatch_get_main_queue(), ^{
      [self.eventSink diagnosticInformationAvailable:@"video" process:nil value:self.filePath];
    });
  }];
}
